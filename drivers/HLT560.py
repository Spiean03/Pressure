...
class Instrument(Tool.MeasInstr): 
    ...
    #1 will enable a watchdog function that will check every telegramm from master to slave and every answer from slave to master
    watchdog=0

    # this is the serial address the GPIB-RS232 converter must use
    __serial_address=''    
    #after serial address, communication must indicate if it is read ('00') or write('10')
    __const_read = '00'
    __const_write = '10'
    
    #Contains the different section of a typical message in the communication protocole with the instrument
    __message_sections=['Address','Action','Parameter#','DataLength','Data','Checksum']   
    __command_list=[]
    #if this value is set to any other values beside 0 it will print the message sections and their value each time the method write or read is called    
    display_infos=0
    
    def __init__(self, resource_name, debug=False, serial_address = '001'): 
        super(Instrument, self).__init__(resource_name,'HLT560',debug)
        
        self.__serial_address=str(serial_address)
        
        self.__command_list=self.__get_param_manual_info()
        
#        self.watchdog=1        
        self.start_time=time.time()
        if not self.debug:
            self.connexion.delay = 0.25 #instrument should be serial port COM#
        
#        print self.identify("Hello, this is ")
            
        #print self.set_PhysUnits('000')  #set the units in mbarl/s for the flow and mbar for pressure                                 # do not confuse with the GPIB address of the converter 
                
#        print "***"        
#        print "The units are mbarl/s for flow and mbar for pressure"
#        print "***"         
#        print "The information about a command # can be found using the method command_info()"
#        print "***"         
#        print "To print more information about the telegrams sent and recieved sent by a get/set function set the attribute display_info to 1"
#        print "***" 
#        print "To print more information about the interpretation of the telegramms set the attribute watchdog to 1"
#        print "***" 
...
#-----------------------------------------------------------------------------    
   #Only for debugging, use the ID of a command and get the answer of the instrument for a read instruction
    def debug(self,ID):
        answer=self.read(self.__msg_read(ID))
        return answer[4]

    #the communication protocol is quite specicic to this instrument so we redefine read and write as private methods
    def read(self,msg):
...
#            print 'Execute : ' +msg
    ...
    #checksum() method calculates the checksum of all the ascii characters recieved in argument if run as own program  
    ...
    #this function is used to prepare the telegrams to communicate with the instrument   
    def message_builder(self,action,param_num,data):
        ...
        msg = self.__serial_address + action + param_num + data_length + str(data) 
        msg=msg+self.__checksum(msg)
        return msg       

    #parse the telegraph style commandline matching the different message_sections
    def message_parser(self,msg,display=0):    

        data_length=int(msg[8:10])
        message=[msg[0:3],msg[3:5],msg[5:8],msg[8:10],msg[10:(10+data_length)],msg[(10+data_length):(13+data_length)]]
...
#        print self.last_measure
            if self.last_measure.has_key(channel):
                if channel=='FLOW':
                    answer=self.get_LeakRate_mbarls()
                elif channel=='P2':
                    answer=self.get_PressureP2()
            ...
            answer=exp_decay(dt,2,50)*(1+0.05*(random.random()-0.5))
        return answer
...
#        print int(abs(expo-23.0))
    ...
    def __get_param_manual_info(self):
        ...
        cmd_file = open("HLT560_commands.txt")
        
        #settings will contain the two dictionnaries, dict1 contains the command name referenced by the command number dict2 contain the command description referenced by the command name
        settings=[];dict1={};dict2={}
        for line in cmd_file:
            if line[0] != '#':
                #the symbol @ separates the field command number, command name, command description
    ...
    def __communication_watchdog(self,msg_sent,msg_recieved,display=1):
        ...
        if msg_in[1]=='00':
            ...
            #possibility to add a test to see whether the size of the data matches the normally sent values
...
#    print 'attempt to rerun this driver'