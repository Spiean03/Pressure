...
class Instrument(Tool.MeasInstr):  
    ...
    def __init__(self, resource_name, debug=False): 
        super(Instrument, self).__init__(resource_name,'CG500',debug)
        
#        print self.identify("Hello, this is ")
        #self.const_label='CG500'
        if self.debug == False:
            self.set_alarm(0)
            self.set_units('CM')
            self.set_mode('S')
...
#        else:
#            pass
#        print "a " + self.ID_name + " is dead"
#        

#------------------------------------------------------------------------------
    
    #I don't know if the values measured stack or not, if they do we should probably think about this function again
    ...
    #0 will desactivate the alarm, a low value will not annoy people in the lab     
